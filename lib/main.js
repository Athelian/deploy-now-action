...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const deployment = yield deploy(`${repoDir}/${buildDir}`, zeitToken, options);
            core.debug(options)
            core.debug("Hi Eliot")
            core.debug(repoDir)
            core.debug(buildDir)
            yield postComment(formatDeployment(deployment, options.name), githubToken);
        }
        catch (error) {
...
function deploy(path, token, options) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            for (var _b = __asyncValues(now.createLegacyDeployment(path, Object.assign({ token }, {...options, forceNew: true}))), _c; _c = yield _b.next(), !_c.done;) {
...
function formatDeployment(deployment, name) {
    // @ts-ignore
